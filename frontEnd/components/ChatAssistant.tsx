...
import { Send, Bot, Loader2, RefreshCw, PenTool, Box } from 'lucide-react';
...
const EngineeringAssistant: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    setInputValue('');
    setMessages(prev => [...prev, { role: 'user', text: userMsg, username: user?.username }]);
    setIsLoading(true);
          ...
          <div className="p-4 bg-white dark:bg-dark-card border-t border-gray-100 dark:border-gray-700">
            {user ? (
              ...
              <div className="text-center text-lg text-gray-600 dark:text-gray-300 py-8">
                <input type="text" placeholder='يجب تسجيل الدخول أولاً لاستخدام الشات.' className="bg-gray-100 dark:bg-gray-500 text-gray-900 dark:text-white rounded-lg px-4 py-2 w-full max-w-md mx-auto" readOnly />
              </div>
            )}
          </div>
        </div>
