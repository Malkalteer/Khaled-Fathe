...
const Evaluation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchReviews()
      ...
      .catch(() => setError('تعذر تحميل التعليقات'))
    ...
    <section id="evaluation" className="py-20 bg-gray-50 dark:bg-dark-bg transition-colors">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex justify-center my-8">
          {user ? (
            <button
              className="bg-primary-500 text-white px-6 py-2 rounded-md text-lg hover:opacity-90"
              onClick={() => navigate('/add-review')}
            >
              إضافة تعليق وتقييم
            </button>
          ) : (
            <div className="text-gray-600 dark:text-gray-300 text-lg">سجّل الدخول لإضافة تعليقك وتقييمك</div>
          )}
        </div>
        {loading ? (
          <div className="text-center text-gray-500 dark:text-gray-400">جاري تحميل التعليقات...</div>
        ) : error ? (
          <div className="text-center text-red-500">{error}</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {reviews.map((r, idx) => (
              <article key={r._id || idx} className="bg-white dark:bg-dark-card border border-gray-100 dark:border-gray-700 rounded-2xl p-5 shadow-sm">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-full bg-primary-50 text-primary-600 flex items-center justify-center font-bold text-sm">{r.username?.slice(0,1)}</div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-semibold text-gray-900 dark:text-white">{r.username}</h4>
